...
read_peptide_csv_generic_bysamp <- function(peptide_file) {
  ...
  peptides <- read.csv(peptide_file, fileEncoding="UTF-8-BOM")
  print(names(peptides)[[1]])
  if (any(grepl("[^ -~]", names(peptides)))){
    print("Alternate read used")
    peptides <- read.csv(peptide_file)
  }
...
plot_intensity <- function(AA_df, protein, intensity_label = "PSM",
  ...
  if (plot_type == "bar") {
    ...
    plot <- ggplot(data = AA_df) +
      ...
      geom_point(aes(x = AA_index, y = intensity, AA = AA),
                 size = 0, alpha = 0) +
      theme_bw(base_size = font_size) +
...
plot_origin <- function(AA_df, protein,
  ...
  if (plot_type == "bar") {
    ...
    plot <- ggplot(data = AA_df) +
      geom_line(aes(x = AA_index, y = intensity),
                color = color, alpha = alpha) +
      geom_point(aes(x = AA_index, y = intensity, AA = AA, origin_pep = origin_pep),
                 size = 0, alpha = 0) +
      theme_bw(base_size = font_size) +
      theme(panel.grid = element_blank(), legend.position = "right") +
      labs(title = protein, x = "Amino Acid Number", y = intensity_label)+
